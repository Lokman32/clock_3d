...
function BasicRing({ scale, speed = 0 }) {
  ...
  return (
    <mesh ref={ringRef} scale={scale}>
      <torusGeometry args={[1, 0.05, 5, 12]} />
      <meshStandardMaterial color="gold" side={2} wireframe />
    </mesh>
  );
}
  ...
  return (
    <Canvas camera={{ position: [0, 0, 6], fov: 50 }}>
      ...
      <group rotation-y={Math.PI * 0.15}>
        ...
        <ClockWise scale={.9} speed={2} color="green" position={[0, 0.36, 0.01]} />
        <ClockWise scale={.7} speed={3} color="blue" position={[0, 0.28, 0.02]} />
      </group>
